...
async function copyDataToSheets(sourceSpreadsheetId, destinationSpreadsheetId) {
  ...
  try {
    ...
    if (sourceSpreadsheetId) {
      ...
      for (const sourceSheet of sourceSheets) {
        if (!sourceSheet.data || !sourceSheet.data.length) continue;

        // Extract data from the source sheet
        const rowData = sourceSheet.data[0].rowData;
        if (!rowData) continue;

        const dataToCopy = rowData.map((row) =>
          row.values ? row.values.map((cell) => (cell.formattedValue ? cell.formattedValue : '')) : []
        );

        // Get the title of the source sheet
        const sourceSheetTitle = sourceSheet.properties.title;

        // Find the matching destination sheet in the newly created spreadsheet
        const matchingSheet = await sheets.spreadsheets.get({
          spreadsheetId: destinationSpreadsheetId,
          ranges: [sourceSheetTitle],
          includeGridData: true,
        });

        if (!matchingSheet || !matchingSheet.data.sheets || matchingSheet.data.sheets.length === 0) continue;

        // Extract data from the destination sheet
        const destinationRowData = matchingSheet.data.sheets[0].data[0].rowData;
        const destinationData = destinationRowData.map((row) =>
          row.values ? row.values.map((cell) => (cell.formattedValue ? cell.formattedValue : '')) : []
        );

        // Append the source data to the destination sheet
        destinationData.push(...dataToCopy);

        const startRowIndex = destinationRowData.length - 1;
        const endRowIndex = startRowIndex + dataToCopy.length;

        const requests = [
          {
            pasteData: {
              coordinate: {
                sheetId: matchingSheet.data.sheets[0].properties.sheetId,
                rowIndex: startRowIndex,
                columnIndex: 0,
              },
              data: dataToCopy.map((row) => row.join('\t')).join('\n'),
              type: 'PASTE_NORMAL',
              delimiter: '\t',
            },
          },
        ...
        console.log(`Data copied to ${sourceSheetTitle}`);
      }
    }

    console.log('Google Sheets created and data copied successfully.');
  } catch (err) {
...
async function createSpreadsheetWithSheets(title, month, year, sampleSheetId) {
  ...
  try {
    ...
    // If sampleSheetId is provided, copy data from the sample sheet to each new sheet
    if (sampleSheetId) {
      await copyDataToSheets(sampleSheetId, spreadsheetId);
    }
